...
public class MyAvlMap implements Map<Integer, String> {
    ...
    public Set<Entry<Integer, String>> entrySet() {
        ...
        inOrderEntrySet(root, entrySet);
        return entrySet;
    }

    private void inOrderEntrySet(Node node, Set<Entry<Integer, String>> entrySet) {
        if (node != null) {
            inOrderEntrySet(node.left, entrySet);
            entrySet.add(new AbstractMap.SimpleEntry<>(node.key, node.value));
            inOrderEntrySet(node.right, entrySet);
        }
    }
    ...
    public Set<Integer> keySet() {
        ...
        inOrderKeySet(root, keySet);
        return keySet;
    }

    private void inOrderKeySet(Node node, Set<Integer> keySet) {
        if (node != null) {
            inOrderKeySet(node.left, keySet);
            keySet.add(node.key);
            inOrderKeySet(node.right, keySet);
        }
    }
