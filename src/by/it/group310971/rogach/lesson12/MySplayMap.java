...
public class MySplayMap implements NavigableMap<Integer, String> {
    ...
    private static class Node {
        ...
        Node parent, left, right; // Родитель, левый и правый дочерние узлы

        public Node(Integer key, String value) {
            this.key = key;
            this.value = value;
            this.left = this.right = this.parent = null; // Инициализация узлов
        }
    }
    ...
    private Node leftRotate(Node n) {
        Node child = n.right; // Правый дочерний узел
        if (child != null) {
            n.right = child.left; // Перемещение левого поддерева
