...
public class MySplayMap implements NavigableMap<Integer, String> {
    ...
    private Node splay(Node node, Integer key) {
        ...
        if (cmp < 0) {
            if ((cmp = key.compareTo(target.key)) > 0) {
                ...
                if (target.right != null) node.left = target.rotateLeft();
            } else if (cmp < 0) {
        ...
        if ((cmp = key.compareTo(target.key)) < 0) {
            target.left = splay(target.left, key);
            if (target.left != null) node.right = target.rotateRight();
        } else if (cmp > 0) {
            target.right = splay(target.right, key);
    ...
    public String remove(Object key) {
        if (root == null || key == null || !(key instanceof Integer)) return null;
        root = splay(root, (Integer) key);
        if (root == null || ((Integer) key).compareTo(root.key) != 0) return null;
    ...
    public String get(Object key) {
        if (root == null || !(key instanceof Integer)) return null;
        var node = root.get((Integer) key);
